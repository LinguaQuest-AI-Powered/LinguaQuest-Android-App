...
class MarkLockScreenWordOpenedUseCase @Inject constructor(
    private val repository: LockScreenRepository
) {
    suspend operator fun invoke(wordId: Int): LinguaQuestResult<Unit, LinguaQuestDataError> {
        return repository.markOpened(wordId)
    }
}
